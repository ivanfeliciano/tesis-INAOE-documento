...
2) en un paso de tiempo $t$ del episodio el agente interactúa con su ambiente al realizar una acción $a_t$ (de acuerdo a lo que le diga su política $\epsilon$ greedy) y obtiene una señal de recompensa $r_{t+1}$ y una observación $s_{t+1}$, y finalmente 3) actualiza los valores
...
De manera general, el algoritmo DQN tiene los siguientes pasos: 1) se inicializan los parámetros para las redes neuronales $Q(s, a)$ y $\hat{Q}(s, a)$ con pesos aleatorios y 
...
para cada tupla de transición $j$ se calcula $y_j$, 6) se calcula la pérdida con los
...
\begin{figure}
    ...
    \caption{Ejemplo de un grafo causal $\mathcal{D}$.}
