...
En la siguiente sección se formaliza el problema de aprendizaje por refuerzo como
...
\begin{itemize}
    ...
    \item $\mathcal{P}_{ss'}^{a}: \mathcal{S}\times \mathcal{A} \times \mathcal{S} \rightarrow [0, 1]$ es una función de probabilidad de transición de pasar del estado $s \in \mathcal{S}$ a $s' \in \mathcal{S}$ cuando se toma la acción $a$.% en el tiempo $t$.
...
\end{itemize}
% Además de estos elementos, uno puede identificar otros subelementos en un sistema de aprendizaje por refuerzo: una \textit{política}, una \textit{función de valor}, y a veces un \textit{modelo} del ambiente.
...
% % Aprendizaje por refuerzo aprende las funciones de valor o lapolıtica mientras interactua con el ambient
...
Dados dos conjuntos de variables disjuntos, $X$ y $Y$,
