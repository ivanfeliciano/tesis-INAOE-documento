...
El método propuesto 
% puede trabajar con varios algoritmos
%  de RL que utilicen una política $\epsilon$-greedy 
%  durante su entrenamiento para elegir sus acciones. En
%  este trabajo, 
 se incorpora en el algoritmo Q-learning \cite{watkins1992q}. El proceso de entrenamiento 
...
Con probabilidad $1-\epsilon$ se elige la mejor acción. Para el algoritmo Q-learning la mejor acción está dada por aquella acción que da el mayor valor para la función de valor de acción, esto se denota como $\argmax_a Q(s, a)$.
...
A menudo, la meta final de una tarea en RL está compuesta de submetas. Por lo tanto, la lista $E$ puede guardarse en un estructura de datos tal que sus elementos estén ordenados por una función de prioridad, por ejemplo, de acuerdo con el orden topológico de $\mathcal{D}$. Con esto, los elementos con mayor prioridad pueden ser las variables que llevan a cumplir submetas necesarias para alcanzar la meta final.
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.35]{Chapter4/Figs/listofinterest.pdf}
...
El grafo (b) denota el caso donde para llevar a cabo una acción $a$ es necesario realizar antes la acción $a'$. La estructura (c) muestra el caso donde $e$ solo es afectado cuando varias acciones son ejecutadas al mismo tiempo. Finalmente, el caso atacado en este trabajo es (d), donde la acciones son nodos sin padres y además no es necesario llevar a cabo varias acciones al mimo tiempo para cambiar $e$.
...
\begin{mialgoritmo}[H]
  ...
  \COMMENT{$f$ Obtiene una lista ordenada de aquellas variables diferentes $g_i \neq x_i$, donde $g_i, x_i \in \mathcal{X}$}
    \FOR{$e \in E$}
    \IF{$e \in \mathcal{D}$}
...
El Algoritmo \ref{alg:guided-action-selection} encaja sin problemas en el algoritmo Q-learning y DQN. En los Algoritmos
\ref{alg:q-algo-extra-info} y \ref{alg:dqn-algo-causal}, se pueden ver los métodos resultantes al añadir la selección de acciones guiada. Además, se puede notar que no es necesario seguir el esquema de la política $\epsilon-greedy$. Se podría utilizar otro método basado en política, por ejemplo, donde antes de utilizar la política que se está aprendiendo, se consulte el modelo causal.
...
En ambos algoritmos \ref{alg:q-algo-extra-info} y  \ref{alg:dqn-algo-causal}, el valor de $\epsilon$ se va actualizando con el paso del tiempo. Para este trabajo se utiliza un  Linear Annealing Policy computes a current threshold value and
